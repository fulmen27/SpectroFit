def compute_delim(my_import, num_ordre=None, btn_state=False):
    if my_import.type == "s" or my_import.type == "s":
        # compute limit for Narval file: to delete recovering order
        x_lower = 0
        x_upper = 0
        lower_lim = None
        upper_lim = None

        if btn_state:  # check for full spectrum check or not
            x_lower = 1
            x_upper = -1
            ...
            for i in range(len(my_import.data["lambda"])):  # value in order limits
            ...
            for j in range(int(x_lower), int(x_upper)):  # delete recovering order
                if float(my_import.data["lambda"][j]) > float(my_import.data["lambda"][j+1]):
                    x_lower = j+1
                    break

    elif my_import.type == "fits":
        # in Fit file, limits are already computed
    ...
    for k in range(len(my_import.lineident["lambda"])):  # get data from lists of all known elements for a given order
