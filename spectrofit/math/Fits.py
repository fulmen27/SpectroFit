...
class Fits:
    ...
    def simple_gaussian(self, mod):
        min = self.master.dict_tabs["Tab_{}".format(self.master.tabs.currentIndex())]["lim"][2]
        max = self.master.dict_tabs["Tab_{}".format(self.master.tabs.currentIndex())]["lim"][3]
        moy = (min + max) / 2
        gmodel = Model(mod)
        gmodel.set_param_hint('sigma1', min=0, max=100)
        gmodel.set_param_hint('mu1', min=min, max=max)
        gmodel.set_param_hint('add', min=0)
        self.solution["simple_gaussian"] = gmodel.fit(self.y, x=self.x, sigma1=1, mu1=moy, add=1,
                                                      method='powell')
    ...
    def double_gaussian(self, mod):
        ...
        gmodel.set_param_hint('mu1', min=min, max=max)
        gmodel.set_param_hint('mu2', min=min, max=max)
        self.solution["double_gaussian"] = gmodel.fit(self.y, x=self.x, sigma1=1, sigma2=1, mu1=moy1, mu2=moy2, add=1,
                                                      method='powell')
    ...
    def lorentz(self, mod):
        ...
        gmodel.set_param_hint('mu', min=min, max=max)
        self.solution["lorentz"] = gmodel.fit(self.y, x=self.x, mu=moy, gamma=1, add=1, method='powell')
    ...
    def double_lorentz(self, mod):
        ...
        gmodel.set_param_hint('mu', min=min, max=max)
        gmodel.set_param_hint('mu2', min=min, max=max)
        self.solution["double_lorentz"] = gmodel.fit(self.y, x=self.x, mu=moy1, gamma=1, mu2=moy2, gamma2=1, add=1,
                                                     method='powell')
...
def model_double_gaussian_emission(x, sigma1, mu1, sigma2, mu2, add):
    return 1 / (sigma1 * np.sqrt(2 * np.pi)) * np.exp(- 1 / 2 * ((x - mu1) / sigma1) ** 2) \
           + 1 / (sigma2 * np.sqrt(2 * np.pi)) * np.exp(- 1 / 2 * ((x - mu2) / sigma2) ** 2) + add
...
def model_double_lorentz_emission(x, mu, gamma, mu2, gamma2, add):
    return (2 / (np.pi * gamma)) / (1 + ((x - mu) / (0.5 * gamma)) ** 2) + \
           (2 / (np.pi * gamma2)) / (1 + ((x - mu2) / (0.5 * gamma2)) ** 2) + add
